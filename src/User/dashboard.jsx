...
const products = [
  { id: 1, name: 'Bamboo Toothbrush', price: 'Rp 29.000', img: '/public/sikatgigi.jpeg' },
  ...
  { id: 5, name: 'Cotton Tote Bag', price: 'Rp 49.000', img: '/product-4.jpg' },
];
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      ...
      const cartCount = parsedCart.reduce((total, item) => total + item.quantity, 0);
  ...
  const handleSubscribe = () => {
    ...
    if (!isValid) {
      ...
      setToast({ open: true, message: 'Successfully subscribed!', variant: 'success' });
      setEmail('');
    }
    clearTimeout(window._db_toast_timer);
    window._db_toast_timer = setTimeout(() => setToast(t => ({ ...t, open: false })), 2000);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="relative z-10 h-full flex items-center justify-center px-6 text-center">
          <div>
            ...
            <p className="mt-4 text-white/90 max-w-2xl mx-auto">
              Choose your favorite eco-friendly products and start small steps for a better earth.
            </p>
      ...
      <section id="shop" className="max-w-7xl mx-auto px-6 py-14">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-8">Recommendation</h2>
      ...
      <section id="about" className="bg-white">
        <div className="max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-10 items-center">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">About EcoPick</h2>
            <p className="mt-3 text-gray-600">
              EcoPick is here to make it easier for you to switch to more sustainable products without hassle.
              We choose eco-friendly materials, responsible production processes, and minimal plastic packaging.
            </p>
          ...
          <div className="rounded-2xl overflow-hidden shadow">
            <img src="/about.jpg" alt="Eco mission" className="w-full h-full object-cover" loading="lazy" decoding="async" />
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 text-center">What They Say</h2>
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          {testimonials.map(t => (
            <div key={t.id} className="bg-white rounded-2xl shadow-sm p-6">
              <div className="flex items-center gap-3">
                <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover bg-gray-200" loading="lazy" decoding="async" />
                <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-16 text-center">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Start Small Changes Today</h2>
          <p className="mt-2 text-gray-600">Join our newsletter for promotions & sustainable living tips.</p>
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            />
            <button onClick={handleSubscribe} className="px-6 py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white font-medium">
              Subscribe
            </button>
          </div>
          <div aria-live="polite" className="sr-only">{toast.open ? toast.message : ''}</div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16" id="faq">
        <div className="flex items-end justify-between mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Articles & Tips</h2>
            <p className="text-gray-600 mt-1">Learn to live more environmentally friendly.</p>
          </div>
          <Link to="/blog" className="text-green-700 hover:underline">View All</Link>
        </div>
          ...
          {posts.map(p => (
            <article key={p.id} className="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition">
              ...
              <div className="p-4">
                ...
                <Link to={`/blog/${p.id}`} className="inline-block mt-3 text-green-700 hover:underline">Read more</Link>
              </div>
            </article>
          ))}
        </div>
      ...
      {toast.open && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-2 rounded-lg shadow ${toast.variant === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
          {toast.message}
        </div>
      )}
    </div>
